...
def test_dual_simultaneous_failure_injections():
    """Verify core requirement: up to two simultaneous failure injections
    during the same incident session without overwriting.
    """
    # 1. Inject payment failure
    res1 = simulate_payment_failure()
    incident_id_1 = res1.incident_id
    assert "payment_failure" in store.get_active_failures()

    # 2. Inject notification failure in the same session
    ...
    # Both failure events are preserved in the telemetry
...
def test_max_two_simultaneous_failures():
    """Verify that at most two simultaneous failures are kept active."""
    simulate_payment_failure()
    simulate_notification_failure()
    assert store.get_active_failures() == ["payment_failure", "notification_failure"]

    # Injecting a third failure rolls the window to the latest two
    simulate_database_latency()
    active = store.get_active_failures()
    assert len(active) == 2
    assert "notification_failure" in active
    assert "database_latency" in active
    assert "payment_failure" not in active
...
def test_simultaneous_failures_distinguishable():
    ...
    for trace_id, trace_events in traces.items():
        services_in_trace = {e.service for e in trace_events}
